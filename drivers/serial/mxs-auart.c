...
#define MXS_AUART_PORT_DMA_MODE	0x80000000
	...
	int (*dir_set)(unsigned index, int value);
};
...
static inline void mxs_auart_tx_chars(struct mxs_auart_port *s)
{
	...
	struct platform_device *pdev = to_platform_device(s->dev);
	struct mxs_auart_plat_data *plat;
	plat = pdev->dev.platform_data;

	//add by qjw 
	if (plat->dir_set) {
		plat->dir_set(s->port.line,1);//1 output 0 input
	}
