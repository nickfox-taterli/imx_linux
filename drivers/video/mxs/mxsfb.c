...
static int mxsfb_set_par(struct fb_info *info)
{
	...
	info->fix.line_length = get_line_length(info->var.xres_virtual,
 	...
 	mydbg("info->var.xres_virtual=%d\n",info->var.xres_virtual);	
 	mydbg("info->var.bits_per_pixel=%d\n",info->var.bits_per_pixel);	
 	mydbg("info->fix.line_length=%d\n",info->fix.line_length);	
...
static int mxsfb_check_var(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	mydbg( "line_length %d, var->yres_virtual %d, data->mem_size %d\n",
		line_length, var->yres_virtual, data->mem_size);
...
static int __devinit mxsfb_probe(struct platform_device *pdev)
{
	...
	//mxsfb_disable_controller(data);      //modify by hak
	//mxsfb_enable_controller(data);      //modify by hak
	data->cur_phys = data->phys_start;
