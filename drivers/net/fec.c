...
static int fec_enet_mii_probe(struct net_device *dev)
{
	...
	//若PHY0的地址 < PHY1的地址,则按顺序扫描PHY地址 modify by hak
	//for (phy_addr = 0; phy_addr < PHY_MAX_ADDR; phy_addr++) {

	//若PHY0的地址 > PHY1的地址,则需要逆向扫描PHY地址 modify by hak
	for (phy_addr = (PHY_MAX_ADDR - 1); phy_addr >= 0; phy_addr--) {
...
#if 0	// #注释这段代码，解决网线拔下后，网卡不能恢复的bug
	/* Whack a reset.  We should wait for this. */
	writel(1, fep->hwp + FEC_ECNTRL);
	udelay(10);
#endif
