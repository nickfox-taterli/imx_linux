...
{
//EasyARM-iMX283 has no poweron and wp  pin in ssp0 
...
static unsigned long mxs_mmc_setclock_ssp2(unsigned long hz)
{
        ...
        if (hz > 1000000)
                parent = clk_get(NULL, "ref_io.1");//modify by cxf in 2015-03-26
        else
                parent = clk_get(NULL, "xtal.0");
...
static struct mxs_mmc_platform_data mmc0_data = {
	...
	.caps 		= MMC_CAP_4_BIT_DATA 		// modefy by luozhizou 支持inand
				| MMC_CAP_DATA_DDR,
	.min_clk	= 400000,
	.max_clk	= 48000000,
	.read_uA        = 50000,
...
/* modefy by zhuguojun 在阿波罗的287,280,283用spi3	*/
...
#elif defined(CONFIG_SND_SOC_UDA1380) || defined(CONFIG_SND_SOC_UDA1380_MODULE)
void __init mx28_init_audio(void)
{       struct platform_device *pdev;
        pdev = mxs_get_device("mxs-uda1380", 0);
        if (pdev == NULL || IS_ERR(pdev))
                return;
        mxs_add_device(pdev, 3);
        audio_plat_data.saif_mclock = clk_get(NULL, "saif.0");
...
#if 0
static struct easy283_pwm_platform_data easy283_pwm4_platdata = {
	.pwm_ch = 4,
	.default_intensity = 400,
	.max_intensity = 400,
};

static struct easy283_pwm_platform_data easy283_pwm7_platdata = {
	.pwm_ch = 7,
	.default_intensity = 400,
	.max_intensity = 400,
};
...
static struct easy283_pwm_platform_data easy283_pwm3_platdata = {
        .pwm_ch = 3,
        .default_intensity = 400,
        .max_intensity = 65535,
};
...
static struct easy283_pwm_platform_data easy283_pwm4_platdata = {
        .pwm_ch = 4,
        .default_intensity = 400,
        .max_intensity = 65535, /* modify by cxf */
};
...
static void easy283_init_pwm(void)
{
        ...
        for (i = 0; i < plookup->size; i++) {
                ...
                #if defined(CONFIG_iMX_280)             /* modify by cxf */
                case 3:
                        printk("add device pwm chanel 3\r\n");
                        pdev->resource = NULL;
                        pdev->num_resources = 0;
                        pdev->dev.platform_data = &easy283_pwm3_platdata;
                        break;
