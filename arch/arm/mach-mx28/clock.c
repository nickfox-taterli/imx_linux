...
//modify by hak for using ref_pix for lcdif
...
{
	unsigned int reg;
	unsigned int parent = clk->parent->get_rate(clk->parent);
	switch (parent/rate) {
	case 9:
		reg = BM_CLKCTRL_HSADC_RESETB;
		break;
	case 18:
		reg = (1<<BP_CLKCTRL_HSADC_FREQDIV) | BM_CLKCTRL_HSADC_RESETB;
		break;
	case 36:
		reg = (2<<BP_CLKCTRL_HSADC_FREQDIV) | BM_CLKCTRL_HSADC_RESETB;
		break;
	case 72:
		reg = (3<<BP_CLKCTRL_HSADC_FREQDIV) | BM_CLKCTRL_HSADC_RESETB;
		break;
	default:
		pr_err("no divider exist: parent/rate = %u/%u\n", parent, rate);
		return -EINVAL;
	}
...
//	gpmi_set_rate(&gpmi_clk, 50*1000*1000);
}
